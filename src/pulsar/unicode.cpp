...
size_t Pulsar::Unicode::Width(Codepoint code)
{
    if (IsAscii(code)) {
        ...
        IsEastAsianWideCharacter(code) ||
        IsEmojiCharacter(code) ||
        IsGeneralWideCharacter(code)
    ) {
        return 2;
    }

    return !IsGeneralZeroWidthCharacter(code);
}

struct Range{
    Pulsar::Unicode::Codepoint A;
    Pulsar::Unicode::Codepoint B;
};

constexpr size_t EastAsianWideCharacters_Count = 122;
// https://unicode.org/Public/16.0.0/ucd/EastAsianWidth.txt
// Exported widths: W, F
constexpr Range EastAsianWideCharacters[EastAsianWideCharacters_Count] = {
...
constexpr size_t EmojiCharacters_Count = 79;
// https://unicode.org/Public/emoji/16.0/emoji-sequences.txt
// Exported types: Basic_Emoji
constexpr Range EmojiCharacters[EmojiCharacters_Count] = {
...
constexpr Range GeneralZeroWidthCharacters[GeneralZeroWidthCharacters_Count] = {
    ...
    { 0x0890, 0x0891 }, { 0x0897, 0x089F }, { 0x08CA, 0x0903 }, { 0x093A, 0x093C },
    ...
    { 0x110B0, 0x110BA }, { 0x110BD, 0x110BD }, { 0x110C2, 0x110C2 }, { 0x110CD, 0x110CD },
    ...
    { 0x11F3E, 0x11F42 }, { 0x11F5A, 0x11F5A }, { 0x13430, 0x13440 }, { 0x13447, 0x13455 },
...
bool IsContainedInSet(Pulsar::Unicode::Codepoint code, const Range set[], size_t sliceStart, size_t sliceEnd) {
    ...
    Range mid = set[sliceMid];

    if (code < mid.A) {
        return IsContainedInSet(code, set, sliceStart, sliceMid);
    } else if (code > mid.B) {
        return IsContainedInSet(code, set, sliceMid+1, sliceEnd);
    } else {
        return true;
    }
}
